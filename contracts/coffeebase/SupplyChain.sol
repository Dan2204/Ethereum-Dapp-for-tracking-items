...
contract SupplyChain is ConsumerRole, DistributorRole, FarmerRole, RetailerRole, Ownable {
  ...
  // VARIABLE OWNER REMOVED AS OWNER IS NOW USED FROM OWNABLE.SOL //
  ...
  // MODIFIER ONLYOWNER REMOVED AS REDUNDANT NOW INHERITING FROM OWNABLE.SOL //
  ...
  modifier paidEnough(uint _price) { 
    require(msg.value >= _price, "Error: Insufficient eth sent"); 
    _;
  }
  ...
  modifier forSale(uint _upc) {
    require(items[_upc].itemState == State.ForSale, "Error: Not for sale.");
    _;
  }
  ...
  constructor() public payable {
    // OWNER REMOVED AS IS NOW TAKEN FROM OWNABLE.SOL //
    sku = 1;
    upc = 1;
  }

  // KILL FUNCTION MOVED TO OWNABLE.SOL //
  ...
  function harvestItem(
    ...
    public
  {
    // IF FARMER ISN'T MAPPED, ADD THEM //
  ...
  // => COULD ALSO USE "verifyCaller(items[_upc].originFarmerID)" FOR TIGHTER CONTROL?
  ...
  function buyItem(uint _upc) 
    ...
    checkValue(_upc)
    {
      ...
      // uint value = items[_upc].productPrice;
  ...
  // THIS CAN BE MORE SECURE WITH A 'MODIFIER' THAT STATES ONLY THE LISTED 
  // DISTRIBUTOR (NOW THE OWNER) CAN ACCESS THIS.
  // DEPENDANT ON WHETHER OR NOT THE DISTRIBUTOR HAD MULTIPLE STAFF
  // THAT NEED ACCESS, THUS REQUIRING MULTIPLE DISTRIBUTOR ADDRESSES ADDED.
